...
class OnlineSpikingBiVGG(nn.Module):

    def __init__(self, cfg, weight_standardization=True, num_classes=1000, init_weights=True,
                 single_step_neuron: callable = None, light_classifier=True, BN=True, **kwargs):
        super(OnlineSpikingBiVGG, self).__init__()
        self.single_step_neuron = single_step_neuron
    ...
    def _initialize_weights(self):
        for m in self.modules():
            ...
            elif isinstance(m, nn.Conv2d) or isinstance(m, ScaledWSConv2d):
                nn.init.kaiming_normal_(m.weight, mode='fan_out', nonlinearity='relu')
                if m.bias is not None:
                    nn.init.constant_(m.bias, 0)
        ...
        for v in cfg:
            if v == 'M':
                ...
                if weight_standardization:
                    if first_conv:
                        conv2d = QuantConv2d(in_channels, v, kernel_size=3, padding=1, stride=stride)
                        first_conv = False
                    else:
                        if grad_with_rate:
                            conv2d = WrapedSNNOp(QuantConv2d(in_channels, v, kernel_size=3, padding=1, stride=stride))
                        else:
                            conv2d = QuantConv2d(in_channels, v, kernel_size=3, padding=1, stride=stride)
                    layers += [conv2d, neuron(**kwargs), Scale(0.274)]
                else:
                    if first_conv:
...
def _spiking_vgg(arch, cfg, weight_standardization, pretrained, progress, single_step_neuron: callable = None, **kwargs):
    ...
    model = OnlineSpikingBiVGG(cfg=cfgs[cfg], weight_standardization=weight_standardization, single_step_neuron=single_step_neuron, **kwargs)
    if pretrained:
